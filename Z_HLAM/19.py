...
#     *,
#     interval: float = 0.15,   # 150–200 ms
#     timeout: float = 5.0,
#     attempts: int = 20,
# ) -> None:
#     """
#     Fast refresh with early-exit on first cache change.

#     • frequent retries
#     • single interval parameter
#     • exits immediately when snapshot changes
#     """
...
#         # --- first successful change → exit ---
#         if prev_hash is not None and cur_hash != prev_hash:
#             return

#         prev_hash = cur_hash

#         # --- optional: if at least one refresh succeeded, allow early settle ---
...
#             if texts or self._pnl_results: self._last_log_flush_ts = now_ts
#             if texts:
...
#         monitors: Dict[int, PosMonitorFSM],
#     ):
#         if mev.sig_type not in ("copy", "log", "manual"):
#             return
...
#             if mev.sig_type == "manual" and cid not in self.mc.cmd_ids:
#                 continue
...
#                 # if mev.sig_type == "manual" and mev.closed and mev.symbol is None:
