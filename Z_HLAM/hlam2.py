...
#             pos_data["pending"] = True
    ...
    # def get_user_config_snapshot(self, id: int) -> dict:
    #     """Глубокая копия конфига, чтобы воркеры не модифицировали оригинал."""
    #     cfg = self.copy_configs.get(id)
    ...
    #     """
    #     # мастер
    #     if 0 not in self.ctx.copy_configs:
    #         cfg = MASTER_TEMPLATE.copy()
    #         cfg["id"] = 0
    #         self.ctx.copy_configs[0] = cfg

    #     # копии
    #     for cid in range(1, COPY_NUMBER + 1):
    #         if cid not in self.ctx.copy_configs:
    #             cfg = COPY_TEMPLATE.copy()
    #             cfg["id"] = cid
    #             cfg["created_at"] = None
    #             cfg["enabled"] = False
    #             self.ctx.copy_configs[cid] = cfg
